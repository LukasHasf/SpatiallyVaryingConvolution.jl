@testset "Test behaviour of convolutions" begin
    function convolve(x,y)
        Ny,Nx = size(x)
        mask = findall(SpatiallyVaryingConvolution.pad2D(ones(Float64, Ny, Nx)) .==1)
        x = SpatiallyVaryingConvolution.pad2D(x)
        y = SpatiallyVaryingConvolution.pad2D(y)
    ...
    @testset "Convolution with non-varying PSF is normal convolution" begin
        ...
        center = Int.([Ny÷2+1, Nx÷2+1])
        for shift_index in 1:nrPSFs
            psfs[center[1] + shifts[1, shift_index], center[1] + shifts[1, shift_index], shift_index] = one(Float64)/4
